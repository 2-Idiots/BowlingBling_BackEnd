...
public class LessonCommentController {
    ...
    @GetMapping
    public ResponseEntity<Page<LessonCommentResponseDto>> getComments(@PathVariable Long lessonId) {
        Page<LessonCommentResponseDto> comments = lessonCommentService.getComments(lessonId, Pageable.ofSize(10));
        return ResponseEntity.ok(comments);
    }

    @Operation(summary = "댓글 작성", description = "특정 레슨에 댓글을 작성합니다.")
    @PostMapping("/save")
    public ResponseEntity<String> saveComment(@PathVariable Long lessonId, @RequestBody LessonCommentRequestDto requestDto, @AuthenticationPrincipal User user) {
    ...
    public ResponseEntity<String> updateComment(@PathVariable Long lessonId, @PathVariable Long commentId, @RequestBody LessonCommentRequestDto requestDto, @AuthenticationPrincipal User user) {
        String memberEmail = user.getUsername();
